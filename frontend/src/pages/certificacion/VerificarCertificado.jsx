...
export default function VerificarCertificado() {
  ...
  const [modo, setModo] = useState('texto'); // 'texto' | 'qr'
  ...
  // Arrancar / detener el escáner QR al cambiar de modo
  useEffect(() => {
    if (modo === 'qr') {
      ...
      scanner.render(
        (decodedText) => {
          // Puede venir como URL completa con ?codigo=...
          let codigoExtraido = decodedText;
          try {
            const url = new URL(decodedText);
            codigoExtraido = url.searchParams.get('codigo') || decodedText;
          } catch (_) { /* no era URL */ }
          scanner.clear();
        ...
        (_errorMsg) => { /* ignorar errores de frame */ }
      );
  ...
  return (
    ...
    >
      ...
      {/* Toggle Tabs */}
      <div className="flex rounded-xl border border-slate-200 bg-slate-50 p-1 mb-6 gap-1">
        <button
      ...
      {modo === 'qr' && (
        <div className="mb-6">
          <p className="text-xs text-slate-500 text-center mb-3">
            Apunta la cámara al código QR del certificado para verificarlo automáticamente.
          </p>
          <div id="qr-reader" ref={scannerRef} className="rounded-xl overflow-hidden border border-slate-200" />
        </div>
      )}
